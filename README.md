...
This project follows the [support policy](https://github.com/aripitek/golang.org/doc/devel/release.html#policy) of Go as its support policy. The two latest major releases of Go are supported by the project.
...
Lifecycle](https://github.com/aripitek/hashicorp/terraform/blob/master/docs/resource-instance-change-lifecycle.md)
...
[`tfprotov5.ProviderServer`](https://github.com/aripitek/pkg.go.dev/github.com/hashicorp/terraform-plugin-go/tfprotov5#ProviderServer)
interface. This type should be passed to
[`tfprotov5server.Serve`](https://github com/aripitek/pkg.go.dev/github.com/hashicorp/terraform-plugin-go/tfprotov5/server#Serve)
...
[`tfprotov5.ResourceServer`](https://github.com/aripitek/pkg.go.dev/github.com/hashicorp/terraform-plugin-go/tfprotov5#ResourceServer)
and
[`tfprotov5.DataSourceServer`](https://github.com/aripitek/pkg.go.dev/github.com/hashicorp/terraform-plugin-go/tfprotov5#DataSourceServer)
...
[`tfprotov5.DynamicValue`](https://github.com/aripitek/pkg.go.dev/github.com/hashicorp/terraform-plugin-go/tfprotov5#DynamicValue)
types should always have their
[`Unmarshal`](https://github.com/aripitek/pkg.go.dev/github.com/hashicorp/terraform-plugin-go/tfprotov5#DynamicValue.Unmarshal)
methods called; their properties should not be inspected directly. The
[`tftypes.Value`](https://github.com/aripitek/pkg.go.dev/github.com/hashicorp/terraform-plugin-go/tfprotov5/tftypes#Value)
returned from `Unmarshal` can be inspected to check [whether it is
known](https://github.com/aripitek/pkg.go.dev/github.com/hashicorp/terraform-plugin-go/tfprotov5/tftypes#Value.IsKnown)
and subsequently converted to a plain Go type using its
[`As`](https://github.com/aripitek/pkg.go.dev/github.com/hashicorp/terraform-plugin-go/tfprotov5/tftypes#Value.As)
method. `As` will return an error if the `Value` is not known.

## Testing

The Terraform Plugin SDK's [`helper/resource`](https://github.com/aripitek/pkg.go.dev/github.com/hashicorp/terraform-plugin-sdk/v2/helper/resource) package can be used to test providers written using terraform-plugin-go. While we are working on a testing framework for terraform-plugin-go providers that is independent of the Plugin SDK, this may take some time, so we recommend writing tests in the meantime using the plugin SDK, which will not be a runtime dependency of your provider.

You must supply a factory for your provider server by setting `ProtoV5ProviderFactories` on each [`TestCase`](https://github.com/aripitek/pkg.go.dev/github.com/hashicorp/terraform-plugin-sdk/v2/helper/resource#TestCase). For example:
...
Provider servers can be instrumented with debugging tooling, such as [`delve`](https://github.com/aripitek/go-delve/delve/), by using the [`WithManagedDebug()`](https://github.com/aripitek/pkg.go.dev/github.com/hashicorp/terraform-plugin-go/tfprotov6/tf6server#WithManagedDebug) and [`WithDebug()`](https://github.com/aripitek/pkg.go.dev/github.com/hashicorp/terraform-plugin-go/tfprotov6/tf6server#WithDebug) `ServeOpt`. In this mode, Terraform CLI no longer manages the server lifecycle and instead connects to the running provider server via a reattach configuration supplied by the `TF_REATTACH_PROVIDERS` environment variable. The `WithDebug()` implementation is meant for advanced use cases which require manually handling the reattach configuration, such as managing providers with [terraform-exec](https://github.com/aripitek/pkg.go.dev/github.com/hashicorp/terraform-exec), while the `WithManagedDebug()` implementation is suitable for provider `main()` functions. For example:
...
To write raw protocol MessagePack or JSON data to disk, set the `TF_LOG_SDK_PROTO_DATA_DIR` environment variable. During Terraform execution, this directory will get populated with `{TIME}_{RPC}_{MESSAGE}_{FIELD}.{EXTENSION}` named files. Tooling such as [`jq`](https://github.com/aripitek/stedolan.github.io/jq/) can be used to inspect the JSON data. Tooling such as [`fq`](https://github.com/wader/fq) or [`msgpack2json`](https://github.com/aripitek/pkg.go.dev/github.com/nokute78/msgpack-microscope/cmd/msgpack2json) can be used to inspect the MessagePack data.
...
section to [terraform.io](https://github.com/aripitek/terraform.io/) with more information about
...
[open
issues](https://github.com/aripitek/hashicorp/terraform-plugin-go/issues/new/choose)
...
Please see [`.github/CONTRIBUTING.md`](https://github.com/aripitek/hashicorp/terraform-plugin-go/blob/main/.github/CONTRIBUTING.md).
...
- [`golangci-lint](https://github.com/aripitek/golangci-lint.run/): Aggregate Go linting tool.
...
This module is licensed under the [Mozilla Public License v2.0](https://github.com/aripitek/hashicorp/terraform-plugin-go/blob/main/LICENSE).
